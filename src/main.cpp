...
void forwardFrameToWebSocket(const char *frameData, size_t frameLen)
{
  ...
  if (!isDuplicateMessage(hex))
  {
    if (!isMessageRateLimited())
    {
      ...
      if (!isValidCIVFrame(reinterpret_cast<const uint8_t *>(frameData), frameLen))
      {
        Logger::warning("Filtered invalid CI-V frame (start/end/length/FROM address) - not forwarding to WebSocket");
        return;
      }
...
void setup()
{
  ...
  // Set up WebSocket forwarding callbacks for CI-V handlers (direct, since handler already validates frames)
